...
embedding_matrix = np.zeros((7543, 300))
not_in_model = 0
in_model = 0
inWords = []
i=0
#将待聚类的词和向量写入文件
fw1 = open('uniqueWords1.vector','w',encoding='utf-8')
...
# 计算包不同类簇个数对应的聚类结果
prefs = [i for i in range(500,3501,500)]
for pref in prefs:
    ...
    fw = open('clusterResult\syns_' + str(pref) + '.txt', 'w', encoding='utf-8')
