...
#所有句子和标签写入文件
def writeFile(sent,posLabel,boundaryLabel,charLabel):
    with open('labeledSent.txt','a',encoding='utf-8') as fw:
        for i in range(len(sent)):
            fw.write(sent[i]+' '+posLabel[i]+' '+boundaryLabel[i]+' '+charLabel[i])
            fw.write('\n')
        fw.write('\n')

#获取四个文件夹下，每个文件的文件名，如“病史特点-1.txt”
import os
from features import speechOfTag,boundary
...
for file in os.listdir('data'):
    ...
    for i in range(1,101):
        ...
        writeFile(sent,posLabel,boundaryLabel,charLabel)
        # exit(0)
